...
def getguestdatabyajaxinform(request):
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            if data.exists():
                return JsonResponse(list(data.values('guestname', 'guestemail', 'guestcity','guestcountry','gueststates','guestidtypes','guestsdetails')), safe=False)
            else:
                return JsonResponse({'error': 'No data found matching the query'}, status=404)
        else:
            return JsonResponse({'error': 'User not authenticated or invalid request method'}, status=400)
...
def getrateplandata(request):
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            if data.exists():
                return JsonResponse(list(data.values('base_price', 'max_persons','additional_person_price','rate_plan_name','rate_plan_code')), safe=False)
            else:
                return JsonResponse({'error': 'No data found matching the query'}, status=404)
        else:
            return JsonResponse({'error': 'User not authenticated or invalid request method'}, status=400)
...
def addpaymentininvoice(request,id) :
    if request.user.is_authenticated:
            ...
            return render(request,'showcreditaddpayment.html',{'active_page':'creditmanage','dueamount':dueamount,'creditid':creditid_id,
                                                               'invcid':invcid})
...
def aminityinvoice(request):
    try:
        if request.user.is_authenticated:
            ...
            invoiceitemsdata = AminitiesInvoiceItem.objects.filter(vendor=user,invoice=invcdata)
            return render(request,'aminityinvoice.html',{'active_page':'aminityinvoice','profiledata':profiledata,'invcdata':invcdata,'invoiceitemsdata':invoiceitemsdata})
...
def addaminitiesinvoice(request): 
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            productprice_str = request.POST.get('productprice', '0')  # Default to '0' if not provided
            productqty_str = request.POST.get('productqty', '0')  # Default to '0' if not provided
            ...
            # Handle product tax
            try:
                producttax = int(producttax_str) if producttax_str and producttax_str.isdigit() else 0
            except (ValueError, TypeError):
                producttax = 0  # Default or handle error

            # Handle product discount
            try:
                productdiscount = int(productdiscount_str) if productdiscount_str and productdiscount_str.isdigit() else 0
            except (ValueError, TypeError):
...
def addmoreaminitiesproductininvoice(request):
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            print("total",total_amount,"subt",subtotal_amount,"tax",tax_amount,"grandt",grand_total,"disc",productdiscount)
            ...
            AminitiesInvoice.objects.filter(vendor=user,id=invcid).update(
                total_item_amount=invctotalamt,discount_amount=invcdiscountamt,subtotal_amount=invcsubtotalamt,gst_amount=invcgstamt,
                sgst_amount=invcsgstamt,grand_total_amount=invcgrandamt
            )
        ...
        if request.user.is_authenticated:
            ...
            if AminitiesInvoiceItem.objects.filter(vendor=user,id=itemid).exists():
                ...
                if AminitiesInvoice.objects.filter(vendor=user,id=invoiceid).exists():
                    invoicedata = AminitiesInvoice.objects.get(vendor=user,id=invoiceid)
                    ...
                    invoicegrandtotalamt = invoicedata.grand_total_amount - itemgrandtotal
                    AminitiesInvoice.objects.filter(vendor=user,id=invoiceid).update(total_item_amount=invoicetotalamt,
                        discount_amount=invoicediscountamt,subtotal_amount=invoicesubtotalamt,gst_amount=invoicegstamt,
                        sgst_amount=invoicesgstamt,grand_total_amount=invoicegrandtotalamt
                    )
...
def saveaminitiesinvoice(request):
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            if paymentmode == "cash":
                AminitiesInvoice.objects.filter(vendor=user,id=invoiceid).update(modeofpayment="cash",sattle=True,
                                cash_amount=totalamt)
            elif paymentmode == "online":
                AminitiesInvoice.objects.filter(vendor=user,id=invoiceid).update(modeofpayment="online",sattle=True,
                                online_amount=totalamt)
            elif paymentmode == "Partly":
                AminitiesInvoice.objects.filter(vendor=user,id=invoiceid).update(modeofpayment="Partly",sattle=True,
                            cash_amount=cashamount,online_amount=onlineamount)
...
def aminityhistory(request):
    try:
        if request.user.is_authenticated:
            user = request.user
            advanceroomsdata = AminitiesInvoice.objects.filter(vendor=user).order_by('-id')
            ...
            return render(request,'aminityhistory.html',{'active_page':'aminityhistory','advanceroomdata':advanceroomdata,"checkdata":'yes'})
        else:
            return redirect('loginpage')
    except Exception as e:
            return render(request, '404.html', {'error_message': str(e)}, status=500)   
...
def aminitiesinvoice(request,id):
    try:
        if request.user.is_authenticated:
            user = request.user
            invcid = id
            if AminitiesInvoice.objects.filter(vendor=user,id=invcid,sattle=True).exists():
                profiledata = HotelProfile.objects.filter(vendor=user)
                invoice_data = AminitiesInvoice.objects.filter(vendor=user,id=invcid)
                invoiceitemdata = AminitiesInvoiceItem.objects.filter(vendor=user,invoice_id=invcid)
                return render(request,'aminityinvoicepage.html',{'profiledata':profiledata,'invoice_data':invoice_data,'invoiceitemdata':invoiceitemdata})
            else:
        ...
        if request.user.is_authenticated and request.method == "POST":
            ...
            if checkindate and checkoutdate:
                try:
                    ...
                    checkindate = datetime.strptime(checkindate, '%Y-%m-%d').date()
                    checkoutdate = datetime.strptime(checkoutdate, '%Y-%m-%d').date()
                    queryset = queryset.filter(invoicedate__range=[checkindate, checkoutdate])
                except (ValueError, TypeError):
                    # Handle invalid date format if necessary
                    pass


            return render(request,'aminityhistory.html',{'active_page':'aminityhistory','advanceroomdata':queryset})
        else:
                return redirect('loginpage')   
    except Exception as e:
            return render(request, '404.html', {'error_message': str(e)}, status=500)   
    ...
    try:
        if request.user.is_authenticated:
            ...
            monthly_sales = AminitiesInvoice.objects.filter(
                vendor=user,
                invoicedate__range=[start_of_month, end_of_month]
            ).order_by('-invoicedate')
            # Calculate the total sales for the current month
            total_sales = monthly_sales.aggregate(total_amount=Sum('grand_total_amount'))['total_amount'] or 0
            ...
            return render(request,'aminityhisales.html',{'active_page':'aminitysales','total_sales':total_sales,
                            'total_gst':total_gst,'total_sgst':total_sgst,'total_cash':total_cash,'online_amount':online_amount})
...
def searchaminitiesinvoicedata(request):
    try:
        if request.user.is_authenticated and request.method == "POST":
            ...
            return render(request,'aminityhisales.html',{'active_page':'aminitysales','total_sales':total_sales,
                            'searchdata':searchdata,'startdate':startdate,'enddate':enddate,'total_gst':total_gst,'total_sgst':total_sgst,'total_cash':total_cash,'online_amount':online_amount})
